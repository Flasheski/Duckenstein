...
static void update_ammo_text(game_data_t *d, weapon_t *w)
{
    char buff[48];

    if (w->is_reloading)
        snprintf(buff, sizeof(buff), "RELOADING  Keys: %d", d->player.keys);
    else if (w->type == WEAPON_GRENADE)
        snprintf(buff, sizeof(buff), "x%d  Keys: %d", w->ammo, d->player.keys);
    else
        snprintf(buff, sizeof(buff), "%d/%d  Keys: %d", w->ammo,
            w->weapon_data->max_ammo, d->player.keys);
    sfText_setString(d->hud->ammo_text, buff);
}
