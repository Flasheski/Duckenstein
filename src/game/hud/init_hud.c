...
static bool init_timer(engine_t *engine, hud_t *hud)
{
    ...
    sfText_setCharacterSize(hud->timer,
        (unsigned int) ((float) engine->window_size.x / 33.75F));
...
static void init_hp_bar_ui(hud_t *hud, const sfVector2f *pos,
    const sfVector2f *size, float scale)
{
    sfRectangleShape_setSize(hud->hp_background, *size);
    sfRectangleShape_setPosition(hud->hp_background, *pos);
    sfRectangleShape_setFillColor(hud->hp_background,
        sfColor_fromRGBA(20, 20, 20, 210));
    sfRectangleShape_setOutlineThickness(hud->hp_background, 2 * scale);
    sfRectangleShape_setOutlineColor(hud->hp_background, sfWhite);
    sfRectangleShape_setPosition(hud->hp_bar, *pos);
    sfRectangleShape_setFillColor(hud->hp_bar, HP_BAR_COLOR);
    sfText_setFont(hud->hp_text, hud->hud_font);
    sfText_setCharacterSize(hud->hp_text,
        (unsigned int) (HUD_AMMO_SIZE * scale));
    sfText_setColor(hud->hp_text, sfWhite);
}

static bool init_hp_bar(engine_t *engine, hud_t *hud)
{
    ...
    hud->hp_background = sfRectangleShape_create();
    hud->hp_bar = sfRectangleShape_create();
    hud->hp_text = sfText_create();
    if (!hud->hp_background || !hud->hp_bar || !hud->hp_text)
        return false;
    init_hp_bar_ui(hud, &pos, &size, scale);
    return true;
}

static bool init_hud_parts(engine_t *engine, game_data_t *data,
    unsigned int mini_map_size)
{
    return init_minimap(data->hud, mini_map_size)
        && init_ammo_texts(engine, data->hud) && init_timer(engine, data->hud)
        && init_hp_bar(engine, data->hud) && init_cursor(engine, data->hud)
...
int init_hud(engine_t *engine, game_data_t *data)
{
    ...
    if (!init_hud_parts(engine, data, mini_map_size)) {
        free_hud(data->hud);
        data->hud = nullptr;
        return ERROR;
    }
